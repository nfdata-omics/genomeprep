...
class Classify(Script.Command):
    ...
    def usage(self, P, out):
        out.write(
            ...
            Classify the specified chromosome position(s) according to the transcripts or genes they belong to. 
            If an argument has the form @filename:col, read regions from column `col' of file `filename' (col defaults to 1).

            Options:

            -d N   | Upstream/downstream distance from gene; if specified, sets -dup and -ddn to the same value (default: {}).
        ...
        with Utils.Output(P.outfile) as out:
            ...
            for reg in RS:
                ...
                if P.mode == "t":
                    ...
                    allclasses = []
                    for g in genes:
                        cls = g.classifyPosition(C, pos, P.updistance, P.dndistance)
                        for cl in cls:
    ...
    #                print allclasses
                    ...
                    self.totals = C.totals
                    if len(genes) == 0:
                        self.add('o')
                    else:
                        for g in genes:
                            c = g.classifyPosition(C, pos, P.updistance, P.dndistance)
                            for x in c:
                                ctype = x.key
...
class Closest(Script.Command):
    ...
    def usage(self, P, out):
        out.write(
            """
    Usage: genes.py closest [options] spec...
    ...
    If regions are read from a file, output consists of each line of the input
    file followed by distance from nearest gene, gene ID, name of gene, strand.

    Output is written to standard output, unless an output file is specified with -o.
            ...
            with P.gl:
                for a in P.args:
                    if a[0] == '@':
                        ...
                        (nin, nout) = self.runFile(filename, out,
                ...
                for reg in regions: # Does nothing if we're in @ mode
                    nin += 1
                    (ID, dist) = P.gl.findClosestGene(reg[0], 
                                                      reg[1], 
                                                      reg[2], 
                                                      transcripts=transcript, 
                                                      biotype=biotype, 
        ...
        with open(infile, "r") as f:
            ...
            for line in c:
                if line[0][0] == '#':
                    continue
                nin += 1
                (ID, dist) = P.gl.findClosestGene(line[chrcol], 
                                                  int(line[startc]), 
                                                  int(line[endc]), 
...
class Annotate(Script.Command):
    ...
    def usage(self, P, out):
        out.write(
            """
            Usage: genes.py annotate [options] infile
