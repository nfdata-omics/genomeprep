...
a single column of values from the ID field."""
    ...
    def findClosestGene(self, chrom, start, end, transcripts=True, biotype=None, tss=False, canonical=False):
        """Find the closest gene to the region chrom:start-end, in either direction.

        Returns a tuple: (gene, distance). Distance can be positive (downstream of `end')
        or negative (upstream of `start'). If `transcripts' is True, look at transcripts
        instead of genes; in this case the return value is (transcript ID, distance).
        If `biotype' is specified, restrict the search to genes with that biotype
        (e.g., protein_coding). If `tss' is True, distances are computed relative
